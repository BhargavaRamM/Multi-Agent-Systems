...
breed [ numbers number ]
breed [ frames frame ]
breed [ popups popup ]

globals [ score won? msg? play-after-victory? qvalue ]

patches-own [ occupied? Qlist ]

turtles-own [ value combined? moveable ]
...
to combine
  ...
  ask one-of other squares with [ value = [ value ] of myself ] [ die ]
...
to check-result
  ...
  if won? = TRUE and play-after-victory? = TRUE and msg? = TRUE [ ask popups [ die ]]
  if won? = TRUE and play-after-victory? = TRUE and msg? = FALSE [ win-game ]
